...
def transit_load_drop_downs(scen1, scen2, scen3, aux):
    scen_list = []
    for scen in [scen1, scen2, scen3]:
        # Validation data only available for scenario runs, check if it exists before adding to available scen list
...
def update_visuals(scen_list, aux):
    def compile_csv_to_dict(filename, scenario_list):
        dfs = list(map(lambda x: pd.read_csv(os.path.join('data', x, filename)), scenario_list))
        dfs_dict = dict(zip(scenario_list, dfs))
        return(dfs_dict)

    def process_dataset(dataset, scen_list, index_col, join_col):
        result = []
        # print(agency)
    ...
    result = process_dataset(dataset, scen_list, index_col='agency', join_col='agency')
    agency_boardings = create_totals_table(result)

    dataset = compile_csv_to_dict('daily_boardings_key_routes.csv', scen_list)
    result = process_dataset(dataset, scen_list, index_col='description', join_col=['description','agency'])
    key_route_boardings = create_totals_table(result)

    dataset = compile_csv_to_dict('light_rail_boardings.csv', scen_list)
    result = process_dataset(dataset, scen_list, index_col='station_name', join_col='station_name')
    light_rail_boardings = create_totals_table(result)
